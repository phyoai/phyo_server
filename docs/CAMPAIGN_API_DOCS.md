...
**Budget Field:**
The `budget` field represents the total campaign budget and is required for all campaigns. This field is used for filtering campaigns by budget range using the `minBudget` and `maxBudget` query parameters.
...
- `minBudget` (optional): Minimum budget filter (filters by campaign budget field)
- `maxBudget` (optional): Maximum budget filter (filters by campaign budget field)
