...
export const handleAsk = async (req: Request<{}, AskResponse, AskRequest>, res: Response<AskResponse>): Promise<void> => {
  ...
  try {
    ...
    console.log('🚀 NEW OPTIMIZED FLOW: Perplexity → Perplexity → Demographics');
    console.log('='.repeat(70));
    ...
    if (extraction.usernames.length > 0) {
      console.log(`  - Usernames: ${extraction.usernames.map(u => '@' + u).join(', ')}`);
    }
    ...
    if (extraction.hasUsernames && extraction.usernames.length > 0) {
      ...
      console.log(`\n🔍 AI SEARCH NEEDED`);
      console.log(`   Using Perplexity to find ${extraction.count} influencer(s)...\n`);

      console.log('[STEP 2] Perplexity searching for influencers...');
      const perplexityResult = await findInfluencersWithPerplexity(prompt, extraction.count || 3);

      if (perplexityResult.usernames.length > 0) {
        console.log(`✅ Perplexity found ${perplexityResult.usernames.length} username(s):`);
        console.log(`   ${perplexityResult.usernames.map(u => '@' + u).join(', ')}`);
        ...
        res.status(404).json({
          ...
          data: [],
          message: 'No influencers found matching your criteria. Please try a different search.',
      ...
      res.status(400).json({
        ...
        result: searchCriteria,
        data: [],
        message: 'Please specify either influencer usernames or search criteria (e.g., "food influencers from Mumbai")',
    ...
    const { results, brightScraperAvailable } = await fetchDemographicsForUsernames(usernamesToFetch);
    ...
    const formattedResults = results.map(result => {
      ...
      return {
        ...
        demographics: {
          gender_distribution: result.data?.gender_distribution || { male: 0, female: 0, unknown: 0 },
    ...
    const freshCount = formattedResults.filter(r => r.success && !r.fromCache).length;
    ...
    if (failCount > 0) {
      console.log(`⚠️  Note: ${failCount} profiles were excluded from results due to fetch failures`);
    }
    console.log('='.repeat(70) + '\n');
